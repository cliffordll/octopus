...
it("shows existing run records without collapsing the section by default", async () => {
  ...
  expect(within(runRecordsRegion).getByRole("button", { name: /展开任务执行/ })).toBeInTheDocument();
  ...
  expect(within(runRecordsRegion).getAllByText("任务执行").length).toBeGreaterThan(0);
  ...
  await userEvent.click(within(runRecordsRegion).getByRole("button", { name: /展开任务执行/ }));

  expect(await within(runRecordsRegion).findByLabelText("任务上下文")).toBeInTheDocument();
  ...
  await userEvent.click(within(runRecordsRegion).getAllByRole("button", { name: /展开任务执行/ }).at(-1)!);

  expect((await within(runRecordsRegion).findAllByLabelText("任务上下文")).length).toBeGreaterThanOrEqual(2);
...
it("shows an issue and records comments and review decisions", async () => {
  ...
  const fetchMock = vi.fn((path: string, init?: RequestInit) => {
    ...
    if (path === "/api/issues/issue-1/children?includeWorkProducts=true" && init?.method === "GET") {
      ...
      return respond({
        parent: issue,
        children: [{ ...childIssue, workProducts: [issue.workProducts[3]], lastCloseout: null }],
        activeChildCount: 1,
        settledChildCount: 0,
        totalChildCount: 1,
  ...
  const workProductsRegion = screen.getByRole("region", { name: "执行产物" });

  expect(workProductsRegion).toHaveTextContent("登录流程 PR");
  expect(workProductsRegion).toHaveTextContent("交付物");
  ...
  expect(workProductsRegion).not.toHaveTextContent("代码提交 abc1234");
  ...
  expect(screen.getByRole("link", { name: "预览" })).toHaveAttribute("href", "/api/assets/asset-product-1/content");

  expect(screen.getByRole("link", { name: "打开" })).toHaveAttribute("href", "https://example.com/pr/42");

  expect(workProductsRegion).toHaveTextContent("不可下载");

  expect(within(workProductsRegion).getAllByRole("button", { name: "删除" })).toHaveLength(2);
  expect(screen.queryByDisplayValue("## 执行步骤")).not.toBeInTheDocument();

  expect(screen.queryByRole("region", { name: "任务文档" })).not.toBeInTheDocument();
  expect(screen.queryByRole("region", { name: "历史文档" })).not.toBeInTheDocument();

  const summaryProductRow = within(workProductsRegion).getAllByText("运行摘要")[0].closest(".issue-work-product-row") as HTMLElement;
  await userEvent.click(within(summaryProductRow).getByRole("button", { name: "删除" }));
...
it("executes an assigned issue through the issue execution route", async () => {
  ...
  const fetchMock = vi.fn((path: string, init?: RequestInit) => {
    ...
    if (path === "/api/heartbeat-runs/run-1/events" && init?.method === "GET") {
      ...
      return respond([
        ...
        { id: 3, runId: "run-1", agentId: "agent-1", seq: 3, eventType: "runtime.text", stream: "stdout", message: '{"type":"step_finish","log":"low value"}', createdAt: "2026-06-02T10:00:02Z" },
    ...
    if (path === "/api/heartbeat-runs/run-1/workspace-operations" && init?.method === "GET") {
      ...
      return respond([
        { id: "op-0", orgId: "org-1", heartbeatRunId: "run-1", phase: "workspace_provision", status: "succeeded", metadata: { preflight: true } },
        { id: "op-1", orgId: "org-1", heartbeatRunId: "run-1", phase: "workspace_provision", status: "running", command: "npm test", stdoutExcerpt: "workspace output", logBytes: 40, metadata: { adapterExecution: true } },
      ]);
  ...
  await ensureRunExpanded(runRecordsRegion, "run-1");
  expect(await within(runRecordGroup).findByRole("group", { name: "运行操作" })).toBeInTheDocument();
  expect(within(runRecordGroup).getByRole("button", { name: "取消运行 run-1" })).toBeInTheDocument();
  const runActionButtons = Array.from(
    (within(runRecordGroup).getByRole("group", { name: "运行操作" }).querySelector(".issue-run-actions") as HTMLElement).querySelectorAll("button"),
  ).map((button) => button.textContent);
  expect(runActionButtons.slice(0, 1)).toEqual(["取消运行"]);
  expect(runRecordGroup).toHaveTextContent("动态刷新中");
  ...
  expect(within(runLogBlock).getByRole("button", { name: "折叠运行日志" })).toBeInTheDocument();

  const taskExecutionToggle = within(runRecordGroup).getByRole("button", { name: /展开任务执行/ });

  expect(taskExecutionToggle).toBeInTheDocument();

  await userEvent.click(taskExecutionToggle);

  expect((await screen.findAllByText("runtime.text")).length).toBeGreaterThan(0);

  expect(screen.queryByRole("heading", { name: "执行输出" })).not.toBeInTheDocument();

  expect(screen.queryByRole("heading", { name: "Agent 关键事件" })).not.toBeInTheDocument();

  expect(screen.queryByText("Agent 关键事件已折叠。")).not.toBeInTheDocument();

  expect(screen.getAllByText("回复内容摘要").length).toBeGreaterThan(0);

  expect(screen.getByText("高级诊断")).toBeInTheDocument();

  expect(screen.getByRole("heading", { name: "运行日志" })).toBeInTheDocument();

  expect(runRecordGroup).toHaveTextContent("persisted run log");

  await userEvent.click(screen.getByText("高级诊断"));

  expect(screen.getByRole("heading", { name: "实时日志增量" })).toBeInTheDocument();

  expect(runRecordGroup).toHaveTextContent("stream log chunk");

  expect(screen.getByRole("heading", { name: "原始数据" })).toBeInTheDocument();

  const outputHeadings = within(runRecordGroup).getAllByRole("heading").map((heading) => heading.textContent);

  expect(outputHeadings.indexOf("Agent 关键事件")).toBeLessThan(outputHeadings.indexOf("执行产物"));

  expect(outputHeadings.indexOf("执行产物")).toBeLessThan(outputHeadings.indexOf("原始数据"));
  ...
  expect(screen.getByRole("button", { name: "展开运行日志" })).toBeInTheDocument();
  ...
  const replySummaries = screen.getAllByText("回复内容摘要");
  ...
  expect(screen.getAllByText("payload only reply").length).toBeGreaterThan(0);

  expect(screen.getByText("结构化回复")).toBeInTheDocument();
  ...
  expect(screen.getAllByText(/Agent 正在处理任务/).length).toBeGreaterThan(0);

  expect(screen.getAllByText(/长回复内容/).length).toBeGreaterThan(0);

  expect(screen.getAllByText(new RegExp(`最终结论`)).length).toBeGreaterThan(0);

  expect(screen.queryByText("low value")).not.toBeInTheDocument();

  expect(screen.getByText("queued output")).toBeInTheDocument();

  expect(screen.getAllByText("workspace output").length).toBeGreaterThan(0);
  expect(screen.getByText("准备任务工作区")).toBeInTheDocument();

  expect(screen.getByText("运行 Agent")).toBeInTheDocument();

  expect(screen.getByText("preflight")).toBeInTheDocument();

  expect(screen.getByText("adapterExecution")).toBeInTheDocument();


  expect(screen.queryByRole("button", { name: /显示全部事件/ })).not.toBeInTheDocument();

  const messageJsonToggle = screen.getByText("消息 JSON");

  const messageJsonDetails = messageJsonToggle.closest("details") as HTMLElement;

  expect(messageJsonDetails).not.toHaveAttribute("open");

  await userEvent.click(messageJsonToggle);

  expect(messageJsonDetails).toHaveAttribute("open");

  expect(screen.getByText(/"log": "low value"/)).toBeInTheDocument();

  expect(screen.getByText('"type": "step_finish"')).toBeInTheDocument();

  await userEvent.click(screen.getByRole("button", { name: "折叠运行 run-1" }));

  expect(within(runRecordGroup).queryByRole("group", { name: "运行操作" })).not.toBeInTheDocument();
  ...
  await userEvent.click(within(runRecordGroup).getByRole("button", { name: "取消运行 run-1" }));
...
it("auto-expands the latest live run and explains silent runtime progress", async () => {
  ...
  const liveRunGroup = within(runRecordsRegion).getByText("run-live").closest(".issue-run-record-group") as HTMLElement;

  await userEvent.click(within(liveRunGroup).getByRole("button", { name: /展开任务执行/ }));

  expect(liveRunGroup).toHaveTextContent("进程 31740 已启动，等待 runtime 输出。");

  expect(liveRunGroup).toHaveTextContent("最近进度：runtime still running");

  await waitFor(() => expect(liveRunGroup).toHaveTextContent("Run stream ended without a final or error event"));

  expect(liveRunGroup).toHaveTextContent("动态刷新中");

  expect(liveRunGroup).not.toHaveTextContent("stream 连接中");
...
it("surfaces operator closeout review activity on the issue page", async () => {
  ...
  const run = {
    ...
    finishedAt: "2026-06-08T10:10:00Z",
  };
  const fetchMock = vi.fn((path: string, init?: RequestInit) => {
    if (path === "/api/orgs/org-1/agents" && init?.method === "GET") {
...
it("does not keep stale closeout review warning after a newer run starts", async () => {
  ...
  const fetchMock = vi.fn((path: string, init?: RequestInit) => {
    ...
    if (path === "/api/orgs/org-1/goals" && init?.method === "GET") return respond([]);
    if (path === "/api/orgs/org-1/heartbeat-runs" && init?.method === "GET") return respond([]);
    if (path === "/api/issues/issue-1/runs" && init?.method === "GET") return respond([oldRun, newRun]);
    if (path === "/api/issues/issue-1/heartbeat-context" && init?.method === "GET") return respond({ issueId: "issue-1" });
    if (path === "/api/issues/issue-1/comments" && init?.method === "GET") return respond([]);
    if (path === "/api/issues/issue-1/attachments" && init?.method === "GET") return respond([]);
    if (path === "/api/issues/issue-1/documents" && init?.method === "GET") return respond([]);
    if (path === "/api/issues/issue-1/work-products" && init?.method === "GET") return respond([]);
    if (path === "/api/heartbeat-runs/run-rerun" && init?.method === "GET") return respond(newRun);
    if (path === "/api/heartbeat-runs/run-rerun/events" && init?.method === "GET") return respond([]);
    if (path === "/api/heartbeat-runs/run-rerun/workspace-operations" && init?.method === "GET") return respond([]);
    if (path === "/api/issues/issue-1/activity" && init?.method === "GET") {
      return respond([
        {
          id: "activity-closeout",
          ...
          agentId: "agent-1",
          runId: "run-closeout",
          details: { attempts: 2, maxAttempts: 2, reason: "missing_closure" },
          createdAt: "2026-06-08T10:10:00Z",
        },
      ]);
    }
...
it("labels cancelled passive follow-up runs explicitly", async () => {
  ...
  expect(within(runRecords).getAllByText("任务执行").length).toBeGreaterThan(0);
...
it("refreshes server registered work products when an issue run succeeds", async () => {
  ...
  await userEvent.click(within(screen.getByRole("region", { name: "执行产物" })).getByRole("button", { name: "展开执行产物 1" }));

  expect((await screen.findAllByText("README 标题文档"))[0]).toBeInTheDocument();

  expect(screen.getByRole("region", { name: "执行产物" })).toHaveTextContent("artifacts/readme-title.md");

  expect(screen.getByRole("link", { name: "下载" })).toHaveAttribute("href", "/api/assets/asset-generated/content");
...
it("retries failed reviewer and passive follow-up runs from their run records only", async () => {
  ...
  let runOperationGroup = within(runRecords).getByText("run-review").closest(".issue-run-record-group") as HTMLElement;

  await userEvent.click(within(runOperationGroup).getByRole("button", { name: "重新执行 Reviewer 评审 run-review" }));
  ...
  runOperationGroup = within(runRecords).getByText("run-followup").closest(".issue-run-record-group") as HTMLElement;

  expect(runOperationGroup).toBeInTheDocument();

  await userEvent.click(within(runOperationGroup).getByRole("button", { name: "重新执行 收尾跟进 run-followup" }));
...
it("hides the live stream log when it duplicates the persisted run log", async () => {
  ...
  expect(screen.getByText("暂无执行环境准备记录。")).toBeInTheDocument();
...
it("shows repeat execution after the latest run succeeded", async () => {
  ...
  await userEvent.click(within(screen.getByRole("region", { name: "执行产物" })).getByRole("button", { name: "展开执行产物 0" }));

  expect(screen.getByRole("region", { name: "执行产物" })).toHaveTextContent("最新运行已成功，但 server 没有登记受管产物。");
