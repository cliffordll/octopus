...
- 用户明确要求执行全量测试时，本地完整验证顺序固定为：
  ...
  3. `uv run pytest`
  4. `uv run pyright .`
- “执行全量测试”默认包含闭环修复：完整验证发现代码或测试缺陷后，应定位并修复、执行相关专项回归，再重新执行完整四步验证，直到全部通过或确认存在无法在当前范围内解决的外部阻塞；不得只汇报失败便结束
- 全量测试授权包含为通过验证所必需的代码和测试修复，但不包含提交授权；修复完成后仍须保持未提交，等待用户明确下达“提交”指令
