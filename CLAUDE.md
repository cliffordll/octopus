...
- 默认交付标准是同时满足：功能正确、分层实现符合仓库规范、对应测试补齐、本次修改范围内的专项验证通过
- 默认只执行与当前修改直接相关的格式检查、静态检查和专项测试；不得自行扩大为全仓库验证
- 只有用户明确下达“执行全量测试”或语义完全等价的指令后，才能运行全仓库测试和完整四步验证；普通的“测试”“验证”“提交”或“直接提交”均不视为全量测试授权
- 对于依赖、锁文件、工具链、本地环境版本或高风险公共基础设施修改，可以建议用户执行全量测试，但在取得明确授权前仍只执行相关专项验证
- 修复 lint 或格式问题时默认只对本次涉及的文件执行 `uv run ruff check <files> --fix` 和 `uv run ruff format <files>`；这些会修改文件，不能替代交付验证结果
- 用户明确要求执行全量测试时，本地完整验证顺序固定为：
  1. `uv run ruff check .`
  2. `uv run ruff format --check .`
  3. `uv run pytest`
  4. `uv run pyright .`
- 未收到全量测试授权时，验证命令必须限定到本次修改涉及的文件、模块或测试用例，并在汇报中列出实际执行范围
- 创建提交不会自动触发新的全量测试；已经完成的验证不得仅因为准备提交而重复执行。用户明确要求“直接提交”时，按指令跳过重复验证
- 未收到“执行全量测试”或语义完全等价的明确指令时，创建提交必须使用 `git commit --no-verify`；禁止使用会触发验证钩子的普通 `git commit`
- 只有用户明确要求全量测试时，才允许设置 `OCTOPUS_FULL_VERIFY=1` 触发提交钩子的完整验证；普通的“提交”“直接提交”不得设置该变量
...
- 普通提交统一使用 `git commit --no-verify`，沿用本轮已完成的专项验证；不得为了创建提交再次执行验证
- 用户明确要求执行全量测试时，可以通过 `OCTOPUS_FULL_VERIFY=1 git commit ...` 运行完整提交门禁，并在提交完成后清除该环境变量
