...
from server.access import AccessDeniedError, AccessPolicyService, PermissionGrantSpec
...
async def list_members(
    ...
    rows = await MemberManagementService(session).list(orgId)
...
async def update_member_permissions(
    ...
    try:
        grants = [
            PermissionGrantSpec(permission=item["permissionKey"], scope=item["scope"])
            for item in validate_member_grants(body)
        ]
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    row = await MemberManagementService(session).replace_permissions(
        orgId,
        memberId,
        grants,
...
def _member_response(row: Any) -> dict[str, Any]:
    return {
        "id": row.id,
        "orgId": row.org_id,
