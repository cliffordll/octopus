...
- Before delegating, design the complete parallel child set, then persist it in one atomic call with `octopus issue create-children "<parent>" --children-json '<JSON array>'`. Do not create sibling tasks one by one. A normal parent rerun must reuse the persisted children returned by this command; use the dedicated retry or replacement commands only for an explicit child retry/replacement. Every child entry requires an explicit `assigneeAgentId`; use `octopus agent list --org-id "$OCTOPUS_ORG_ID" --json` when choosing executors. The parent must wait for those child issues to run and report back. Never create a separate summary, merge, or report child: after all real children settle, the parent continuation reads their outputs and performs the final synthesis itself. Never assign delegated work to the parent agent itself. Do not complete delegated child work inside the parent run and then mark those child issues blocked or cancelled as unnecessary. Use `blocked` only for a real blocker, such as missing information, unavailable permissions, failed dependencies, or a required human/external action. Do not mark the parent done while any child remains open.
