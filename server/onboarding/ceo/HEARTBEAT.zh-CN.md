...
- 检查唤醒上下文：`RUDDER_TASK_ID`、`RUDDER_WAKE_REASON`、`RUDDER_WAKE_COMMENT_ID`。
- 如果 `control-plane agent me --json` 返回 `Agent authentication required`，不要把这次运行当作正常 heartbeat。报告注入认证缺失或无效。不要在运行中请求 `RUDDER_API_KEY`，也不要继续用基于文件的手工绕过方案。
...
如果设置了 `RUDDER_APPROVAL_ID`：

- 使用 `control-plane approval get "$RUDDER_APPROVAL_ID" --json` 和 `control-plane approval issues "$RUDDER_APPROVAL_ID" --json` 检查审批及其关联 issue。
...
- 如果设置了 `RUDDER_TASK_ID`，并且该任务分配给你或把你列为 reviewer，优先处理该任务。
...
- 如果 `RUDDER_WAKE_REASON=issue_passive_followup`，把这次唤醒视为 close-out 治理，而不是新分配任务：检查状态，并留下进展评论、完成、阻塞或明确交接。
...
- 如果 `RUDDER_WAKE_REASON=issue_review_closeout_missing`，把这次唤醒视为 reviewer close-out 治理，并记录一个结构化审查决策。

## 6. 委派

- 使用 `control-plane issue create --org-id "$RUDDER_ORG_ID" ... --json` 创建子任务。始终设置 `parentId` 和 `goalId`。
...
- 当 `RUDDER_RUN_ID` 可用时，变更类 `control-plane` CLI 命令会自动附加它。
