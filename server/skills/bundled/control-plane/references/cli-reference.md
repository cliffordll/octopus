...
- `octopus issue create-children <parent> --children-file <UTF-8 JSON file> [--parent-output-required]` — Persist the complete delegated child set atomically. Child outputs are final by default, but the parent Agent is always resumed to summarize them and close the parent issue. Add `--parent-output-required` only when the current instruction requires a separate parent output. Advanced operators may use `--closeout-policy-file <UTF-8 JSON file>` instead; inline policy JSON is not supported. Each child requires `title` and exactly one of `assigneeAgentId` or `assigneeUserId`; optional fields are `description` and `priority`. Human-assigned children are tracked manually and do not create a Run. A retry in the same parent Run returns that Run's persisted batch. Never test with a placeholder child on a real parent, and do not include a summary/report child.
