...
- `octopus issue create-children <parent> (--children-file <UTF-8 JSON file> | --children-json '<JSON array>') --closeout-mode <parent_summary|child_outputs>` — Persist the complete delegated child set atomically. Prefer `--children-file` to avoid shell escaping problems. Use `parent_summary` when the parent must create a final synthesis, or `child_outputs` when child artifacts are final. Each child requires `title` and `assigneeAgentId`; optional fields are `description`, `priority`, and `reviewerAgentId`. A retry in the same parent Run returns that Run's persisted batch. Never test with a placeholder child on a real parent, and do not include a summary/report child.
