---
name: create-plugin
description: Create new control plane plugins with the current SDK/runtime. Use when scaffolding a plugin package, adding an example plugin, or updating plugin authoring docs.
disable: true
---

# Create a Control Plane Plugin

Use this skill when the task is to create, scaffold, or document a control plane plugin.
...
For a plugin that lives outside the control plane repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.control-plane-sdk/`:

```bash
pnpm --filter @control-plane/create-control-plane-plugin build
node packages/plugins/create-control-plane-plugin/dist/index.js @acme/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/control-plane/packages/plugins/sdk
```
...
- is installed into control plane from an absolute local path during development
