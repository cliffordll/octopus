...
from .directory import RoleDirectoryService
...
class RoleManagementService:
    ...
    async def list_organization(self, org_id: str) -> list[ManagedRole]:
        ...
        return [
            await self._describe(entry.role, entry.display_name) for entry in entries
        ]
    ...
    async def _describe(
        self, role: Role, display_name: str | None = None
    ) -> ManagedRole:
        ...
        if display_name is None:
            display_name = (await self._directory.describe(role)).display_name
        return ManagedRole(
            role=role,
