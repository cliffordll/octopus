...
async def test_review_decision_skips_queued_reviewer_wakeup(
    ...
    assert run.error == "review resolved by human"
    assert run.error_code == "review_resolved_by_human"
...
async def test_review_decision_cancels_running_reviewer_wakeup(
    ...
    assert wakeup.finished_at is not None
    assert wakeup.error == "review resolved by human"
    assert run is not None
    assert run.status == "cancelled"
    assert run.finished_at is not None
    assert run.error == "review resolved by human"
...
async def test_reviewer_decision_does_not_cancel_its_own_run(
    app: FastAPI,
    session: AsyncSession,
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
