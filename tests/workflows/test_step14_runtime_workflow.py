...
async def test_opencode_local_reports_work_products_from_write_events(tmp_path) -> None:
    workspace = tmp_path / "workspace"
    report = workspace / "reports" / "final.md"
    report.parent.mkdir(parents=True)
    report.write_bytes(b"# Final report\n")

    fake_opencode = tmp_path / "fake_opencode_write.py"
    fake_opencode.write_text(
        "\n".join(
            [
                "import json, sys",
                "sys.stdin.read()",
                "print(json.dumps({'type':'step_start','sessionID':'ses_write'}))",
                "print(json.dumps({'type':'tool_use','part':{'type':'tool','tool':'write','state':{'status':'completed','input':{'filePath': r'"
                + str(report)
                + "'}}}}))",
                "print(json.dumps({'type':'tool_use','part':{'type':'tool','tool':'bash','state':{'status':'completed','input':{'command':'octopus issue done ISSUE-1 --comment done --primary-work-product reports/final.md --json'}}}}))",
                "print(json.dumps({'type':'text','part':{'type':'text','text':'Wrote reports/final.md'}}))",
            ]
        ),
        encoding="utf-8",
    )
    ...
    assert result.work_products[0]["title"] == "reports/final.md"
    assert result.work_products[0]["isPrimary"] is True
    assert result.work_products[0]["metadata"]["source"] == "opencode_write_event"
    assert result.work_products[0]["content"] == b"# Final report\n"
...
async def test_openclaw_local_uses_managed_home_and_syncs_credentials(
    ...
    control_plane_name = "octopus.cmd" if os.name == "nt" else "octopus"
    assert home.joinpath(".octopus", "bin", control_plane_name).is_file()
    assert str(home / ".octopus" / "bin") in captured_env["PATH"].split(os.pathsep)
    assert result.result_json is not None
