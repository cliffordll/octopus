...
If another Octopus dev runtime is already running, leave it alone and use a
...
- Never use destructive Git commands in the user's main worktree to make a PR
  checkout succeed.
- Do not stop unrelated Octopus dev sessions just because the default port is in
  use; choose an isolated port instead.
- Keep PR preview data isolated with a dedicated `OCTOPUS_INSTANCE_ID`.
