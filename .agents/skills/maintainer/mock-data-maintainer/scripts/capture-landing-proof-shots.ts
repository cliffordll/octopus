...
const OUTPUT_ROOT = "/tmp/octopus-landing-proof-shots";
...
process.stdin.on("end", async () => {
  const sentinel = prompt.match(/(__OCTOPUS_RESULT_[a-f0-9-]+__)/i)?.[1] ?? "__OCTOPUS_RESULT_TEST__";
  const result = {
    kind: "issue_proposal",
    ...
    structuredPayload: {
      issueProposal: {
        ...
        description: [
          ...
          "",
          "Scope:",
          "- compare Octopus against two alternatives",
...
process.stdin.on("end", async () => {
  ...
  process.stdout.write(JSON.stringify({
    ...
    item: {
      id: "msg-1",
      type: "agent_message",
      text: "Completed the requested launch task and synced progress back to Octopus.",
    },
  }) + "\\n");
  process.stdout.write(JSON.stringify({
    type: "turn.completed",
    result: "Completed the requested launch task and synced progress back to Octopus.",
...
async function resolveRuntimeInfo() {
  for (let attempt = 0; attempt < 120; attempt += 1) {
    ...
    if (serverPort && dbPort) {
      ...
      return {
        baseUrl,
        dbUrl: `postgres://octopus:octopus@${DATABASE_HOST}:${dbPort}/octopus`,
      };
    }
    await delay(1000);
  }
...
async function startServer() {
  ...
  const child = spawn(
    PNPM_BIN,
    ["--filter", "@octopus/server", "dev"],
    {
      cwd: REPO_ROOT,
      env: {
        ...process.env,
        PATH: PATH_PREFIX,
        OCTOPUS_HOME: HOME_DIR,
        OCTOPUS_INSTANCE_ID: INSTANCE_ID,
        OCTOPUS_UI_DEV_MIDDLEWARE: "true",
...
async function seedDemoOrg(api: APIRequestContext, dbUrl: string): Promise<SeedContext> {
  ...
  const importGuideIssue = await createIssue(api, org.id, {
    ...
    description: "Document how a new org can import existing project structure and issues into Octopus.",
    status: "todo",
    priority: "medium",
    assigneeAgentId: supportOps.id,
    labelIds: [labelIds.launch],
  ...
  await fs.writeFile(path.join(workspaceRoot, "README.md"), [
    "# Octopus public beta launch workspace",
    "",
  ...
  await fs.writeFile(path.join(workspaceRoot, "launch", "public-beta-message.md"), [
    "# Public beta message",
    "",
    "Octopus is the operating layer for agent teams. The launch page should show how goals, tasks, approvals, skills, and budgets stay connected while agents work.",
    "",
    "## Review points",
  ...
  await createOrganizationResource(api, org.id, {
    ...
    kind: "url",
    locator: "https://example.com/octopus/pricing-v4",
  ...
  const [runCalendarSource, operatorCalendarSource] = await db.insert(calendarSources).values([
    {
      orgId: org.id,
      type: "agent_work",
      name: "Octopus Agent Work History",
      ownerType: "system",
      status: "active",
...
async function setSelectedOrg(page: Page, orgId: string) {
  ...
  await page.evaluate((value) => {
    window.localStorage.setItem("octopus.selectedOrganizationId", value);
  }, orgId);
}
      ...
      <body>
        <div class="wrap">
          <div class="terminal">
            ...
            <pre><span class="prompt">$</span> npx @octopus/cli@latest start
<span class="ok">Octopus</span>
...
Database   ~/.octopus/instances/dev/db

<span class="prompt">$</span> curl http://127.0.0.1:3100/api/health
{"ok":true,"mode":"local_trusted"}

<span class="muted">Seeded demo org: Octopus public beta launch week</span></pre>
          </div>
          <div class="panel">
...
async function writeManifest() {
  ...
  const manifest = {
    ...
    notes: "Generated from an isolated local_trusted Octopus instance for landing proof shots.",
  };
