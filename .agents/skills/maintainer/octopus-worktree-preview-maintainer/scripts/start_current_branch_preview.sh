...
TMP_BASE="${TMPDIR:-/tmp}/octopus-worktree-preview"
BASE_PORT="${OCTOPUS_PREVIEW_BASE_PORT:-3310}"
BASE_PG_PORT="${OCTOPUS_PREVIEW_BASE_PG_PORT:-55310}"
PORT=""
PG_PORT=""
INSTANCE_ID=""
HOME_DIR=""
DATABASE_URL_VALUE="${OCTOPUS_PREVIEW_DATABASE_URL:-}"
MANAGER="${OCTOPUS_PREVIEW_MANAGER:-auto}"
ACTION="start"
REPLACE=0
WAIT_SECONDS="${OCTOPUS_PREVIEW_WAIT_SECONDS:-90}"

usage() {
  cat <<'EOF'
Usage:
  bash .agents/skills/maintainer/octopus-worktree-preview-maintainer/scripts/start_current_branch_preview.sh [options]
...
Options:
  ...
  --instance-id <id>        Octopus instance id. Defaults to a slug from the current branch.
  --home <path>             OCTOPUS_HOME. Defaults to /tmp/octopus-worktree-preview/<instance-id>/home.
...
The script starts the current Octopus worktree with @octopus/server dev and
...
tmux_session_name() {
  printf "octopus-preview-%s" "$INSTANCE_ID"
}

launchctl_label() {
  printf "com.octopus.preview.%s" "$INSTANCE_ID"
}
...
build_command() {
  ...
  cmd+=" OCTOPUS_HOME=$(shell_quote "$HOME_DIR")"
  cmd+=" OCTOPUS_CONFIG=$(shell_quote "$config_path")"
  cmd+=" OCTOPUS_INSTANCE_ID=$(shell_quote "$INSTANCE_ID")"
  cmd+=" OCTOPUS_WORKTREE_NAME=$(shell_quote "$(branch_name)")"
  cmd+=" OCTOPUS_LOCAL_ENV=dev"
  cmd+=" PORT=$(shell_quote "$PORT")"
  cmd+=" OCTOPUS_UI_DEV_MIDDLEWARE=true"
  cmd+=" OCTOPUS_MIGRATION_PROMPT=never"
  cmd+=" OCTOPUS_MIGRATION_AUTO_APPLY=true"

  if [[ -n "$DATABASE_URL_VALUE" ]]; then
    cmd+=" DATABASE_URL=$(shell_quote "$DATABASE_URL_VALUE")"
  else
    cmd+=" OCTOPUS_EMBEDDED_POSTGRES_PORT=$(shell_quote "$PG_PORT")"
  fi

  cmd+=" $(shell_quote "$pnpm_bin") --filter @octopus/server dev"
  printf "%s" "$cmd"
}
...
print_success() {
  ...
  manager_kind="$1"
  log_path="$2"
  stop_cmd="bash .agents/skills/maintainer/octopus-worktree-preview-maintainer/scripts/start_current_branch_preview.sh --instance-id $INSTANCE_ID --stop"

  echo "Octopus preview is running."
  echo "URL: $(preview_url)"
  echo "Health: $(health_url)"
  echo "Instance: $INSTANCE_ID"
  echo "OCTOPUS_HOME: $HOME_DIR"
  if [[ -n "$DATABASE_URL_VALUE" ]]; then
