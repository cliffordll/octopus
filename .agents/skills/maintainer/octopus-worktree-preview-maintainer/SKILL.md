...
Run the current Octopus checkout as a temporary preview so the user can inspect
...
Octopus's normal `pnpm dev` entrypoint is the canonical development path. In
Codex-managed worktrees under `~/.codex/worktrees/<id>/<repo>`, `pnpm dev`
auto-derives an isolated `OCTOPUS_HOME`, instance id, server port, embedded
PostgreSQL port, and worktree branding when no repo-local `.octopus/` config
...
- The user asks only to stop or clean local dev processes; use
  `stop-octopus-dev-maintainer` instead.
...
- The user simply wants to develop inside the current Codex worktree. Use
  ...
  starting a separate `/tmp/octopus-worktree-preview` instance.
...
`OCTOPUS_EMBEDDED_POSTGRES_PORT`. If embedded PostgreSQL fails in the local
environment, use a dedicated external database for this preview instead of
reusing another Octopus instance's database.
...
- starts `@octopus/server` with Vite dev middleware
...
ports or process groups that belong to another Octopus worktree.
...
databases, or stop other Octopus sessions unless the user explicitly asks.
...
- A health-checked URL is the minimum hand-off; logs and a stop command make it
  usable.
- Never stop unrelated Octopus sessions just because they occupy `3100`.
