...
- route / service 边界不再使用裸 `dict[str, Any]`,而是用本地 `OrgSummary` 占位类型收口
...
HTTP 状态码 `503`。服务端日志记录：

```
INFO:     127.0.0.1:... - "GET /api/orgs HTTP/1.1" 503 Service Unavailable
```
...
- 不引入数据库读写集成测试；当前只补最小骨架测试,覆盖 app 导入、路由注册和 `/api/orgs` guard 行为
