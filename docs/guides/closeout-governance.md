...
- assignee run 不能直接把 issue 改成 `done` 来绕过 reviewer；如果工作完成且有 reviewer，应该进入 `in_review`
- 当前 run 把 issue 改成 `blocked`：`activity_log.action = issue.updated` 且 `activity_log.details.status = blocked`
...
| reviewer | `issue review --decision needs_followup` | 保持评审跟进，不当作最终完成 |

因此，有 reviewer 的 issue 不能用 passive follow-up 替代 reviewer run。passive follow-up 只负责补缺失的 assignee closeout 信号；如果 assignee 已经表达“完成”，系统应该把任务推到 `in_review` 并触发 reviewer，而不是再安排普通收尾跟进。
...
| `issue done` | 无 reviewer 时 `issues.status = done`；有 reviewer 时 `issues.status = in_review` | `activity_log.action = issue.updated`，`details.status = done` 或 `in_review` |
...
9. 有 reviewer 的 issue 被 assignee 直接改成 `done`，导致系统认为已收尾，同时没有创建 reviewer run
