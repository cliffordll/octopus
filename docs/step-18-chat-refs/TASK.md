...
在 Step 16 Chat/Messenger 可用后，补齐消息使用链路中最容易阻塞真实调试的消息持久化边界、chat attachment metadata 和基础 transcript 可见性。

本步骤不是完整 Storage，也不是完整 Observability；它只补齐 Chat 使用 Agent 时必须能看到和引用的最小服务端信息。完整对象上传/下载、work product/run result 深度关联和生产级观测能力分别归 Step 22、Step 19/24。
...
- `packages/database/schema/chats.py` 已有 `chat_messages.structured_payload`，可承载上游式内部 transcript metadata。
- `packages/database/schema/workspaces.py` 已有 `IssueWorkProduct`，但 Step 18 不新增 work product/run result 引用模型；相关查询和深度关联归后续 workspace/observability/storage 阶段。
...
作用：让消息可以引用上游式 chat attachment metadata，为真实任务讨论提供附件上下文。
...
作用：让 UI/CLI 能看到 chat assistant 执行过程中的基础 transcript，不需要伪造 run 或提前实现完整 observability。

状态：已完成上游式 transcript 最小闭环；深度日志、指标和恢复矩阵仍归 Step 19/24。

已实现：

- 新增 `ChatStreamTranscriptEntry` shared type，并让 `ChatMessage.transcript` 成为稳定返回字段。
- runtime stream 事件中的 `transcript_entry` 会被收集并持久化到 assistant message。
- API response 会剥离内部 `structuredPayload.__chatTranscript` metadata，只返回干净 `structuredPayload` 和单独 `transcript`。
- 覆盖 stream transcript 事件、最终 assistant message hydration、消息列表刷新后 transcript 可查询测试。

上游证据修正：

- 上游 `ChatMessage` 包含 `transcript?: ChatStreamTranscriptEntry[]`。
- 上游通过内部 key `__chatTranscript` 把 transcript 存在 message `structuredPayload` 中。
- 上游查询 message 时使用 `stripChatMetadataFromPayload()` 剥离内部 metadata，并用 `chatTranscriptFromPayload()` 单独返回 `transcript`。

设计：

- visibility 信息挂在 assistant chat message 的 `transcript` 字段中，用于表达 runtime 的 thinking/tool/result/stdout/stderr 等基础过程事件。
- 不提前实现完整 observability store；stdout/stderr 深度日志、指标和恢复矩阵归 Step 19/24。
- 对外字段必须可由上游 chat 类型或服务行为解释；不得扩展成新的复杂状态机。
- Chat assistant invocation 不生成 heartbeat run，也不写入 run 表。

计划：

1. 对照上游确认 chat assistant transcript 在 message response 中的字段表达。
2. 在 shared type 中增加 `ChatStreamTranscriptEntry` 与 `ChatMessage.transcript`。
3. 在 runtime stream success 分支收集 `transcript_entry`，并随 assistant message 持久化。
4. 在 message list/detail 中返回 `transcript`，并剥离内部 `structuredPayload.__chatTranscript`。
5. 增加 contract tests 覆盖 stream transcript 事件和刷新后 message hydration。

## 边界

- 不实现完整上传、下载、对象存储 lifecycle；完整 Attachment / Storage 归 Step 22。
- 不实现 work product/run result 深度引用；workspace operation、work product 关联查询和 runtime result 观测归 Step 19/22/24。
...
- Tests 覆盖消息携带 attachment metadata 后的 payload、排序和 scope。
- Tests 覆盖消息触发 agent 后可以查询到基础 execution visibility。
- Tests 覆盖引用不存在、跨 organization/conversation 引用的错误行为。
