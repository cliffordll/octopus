...
- 插件 secrets/env 解析依赖 Step 25 的 actor/access/secret 边界；未完成前只能提供明确的开发期兼容行为。
- 插件 cost/budget/activity 归集依赖 Step 20/23/24 已有 observability、成本、activity 与治理记录，不在本步骤重新定义治理模型。
