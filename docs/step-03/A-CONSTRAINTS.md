...
- `GET /api/issues` 在兼容语义上保留错误入口角色，不能被 B 当成正常列表接口随意扩写
...
- `packages/shared/src/` 内不放数据库字段映射，不放 ORM 模型，不放事务逻辑
...
- 在 `server/src/routes/` 中把第一批 path 常量切到 shared
- 在 `server/src/routes/` 中接入第一批 validator
- 在 `server/src/services/` 中消费共享请求 / 响应类型
...
- 第一批 validator 已经存在并可被 route 直接调用
- `server/` 至少有一个第一批 route 已经接到 shared path、shared type 和 shared validator

## 验收 Demo

- Demo 1：展示 `packages/shared/src/api_paths/`、`constants/`、`types/`、`validators/` 的实际文件结构
- Demo 2：选一个 route，展示 path 常量、payload 类型和 validator 已全部改从 shared 引用
