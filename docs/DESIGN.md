...
Octopus 的目标不是重新发明一个新产品，而是用 Python 对 Rudder 的服务端做一次兼容式重写。在这次重写中，核心约束已经非常明确：

- 保持 Rudder 现有数据库表结构不变
- 保持 Rudder 现有 API 接口不变
- 保持 Rudder 现有控制面语义不变
- 支持多个 pod 连接同一个数据库
- 每个 pod 只处理自己独占的一组 organization，绝不交叉
- pod 控制、organization 分配、外层代理服务由其他项目实现，不在 Octopus 项目内实现
- auth 相关能力由外层系统负责，本项目不实现完整认证与鉴权体系
- 仓库目录结构尽量参考 Rudder，允许轻微改名，但不重造一套完全不同的结构

因此，Octopus 的本质不是新系统设计，而是一次带有明确兼容边界的重写工程。

## 2. 背景与问题定义

Rudder 当前是一个以 Node/TypeScript 为主的控制平面系统，负责组织、agent、issue、chat、approval、run、cost、budget、activity 等核心对象的编排与治理。
...
换句话说，Octopus 必须尽可能表现得像今天的 Rudder：
...
## 3.2 以 Rudder 为契约源，而不是以 Python 习惯为契约源

Octopus 的契约来源不是“Python 世界里怎么写更优雅”，而是：

1. Rudder 当前数据库 schema
2. Rudder 当前 API 行为
3. Rudder 当前产品语义
...
Rudder 的业务天然是 organization-scoped。Octopus 必须保持这一点：
...
这类表属于基础设施状态，不属于 Rudder 的业务表重构，因此不违背“业务表结构保持不变”的要求。

## 6.4 外层系统的职责边界

Octopus 之外通常会存在一个外层控制系统，它不是 Rudder 业务语义的实现层，而是 Octopus 的外层编排与代理层。
...
Octopus 的 HTTP 接口命名必须与 Rudder 保持一致。

这不仅包括对外业务接口，也包括任何需要通过 HTTP 暴露的接口面命名约束：

- 不新增一套 Octopus 自定义命名风格
- 不引入 `/internal/...` 这类 Rudder 中不存在的新接口名前缀
...
- 如果 Rudder 已有对应接口，则继续复用 Rudder 的接口名
- 如果 Rudder 没有对应接口，则当前设计阶段不在 Octopus 中新增新的 HTTP 接口名
...
你明确要求目录结构参考 Rudder，因此 Octopus 不应创造一套完全 Python 化但与 Rudder 无关的结构。
...
这里使用 `docs/` 是因为你明确要求设计文档放在 `docs/` 下。除此之外，整体骨架仍然尽量贴近 Rudder。
...
这是整个仓库最核心的目录，对应 Rudder 的 `server/`。
...
`server/` 负责决定何时调用 runtime，以及调用后在 Rudder 业务上意味着什么。
...
它对应 Rudder 的 `packages/db/` 这一职责边界，但在 Octopus 中目录名统一使用 `packages/database/`，因为这个名字更直观。

这一层的目标不是重新设计 schema，而是：

- 用 Python 正确映射当前 Rudder schema
...
它对应 Rudder 的 `packages/shared/`，在 Octopus 中仍然有必要保留。
...
这一层应参考 Rudder 当前的真实实现方式：按 runtime 类型拆成多个包，而不是做成一个大一统目录。

它负责定义 Octopus 如何与不同 agent runtime 通信，而不负责决定这些调用在 Rudder 业务上意味着什么。
...
这样可以把“怎么和 runtime 说话”与“什么时候调用 runtime、调用后如何修改业务状态”分开，也更利于和 Rudder 的现有 runtime 包逐个对照迁移。
...
这相当于 Rudder 服务端启动入口的 Python 版本。
...
这样更容易与 Rudder 的 route surface 对齐，也更利于接口盘点。

这一层只承载 Rudder 兼容业务接口，不混入外层系统专用管理接口。
...
Rudder 有共享 API path 的概念，Octopus 应保留这个边界，避免 route path 在前后端或测试中散落。
...
参考 Rudder 现有实现，runtime 不是“一个总包 + 一堆文件”，而是：
...
参考 Rudder 的真实结构，`codex-local` 这类 runtime 包内部可能还需要自己的 `shared/`，用于该 runtime 独有但又会被 server/ui/cli 共同消费的逻辑。
...
如果后续需要前端代码迁移，则建议继续保持和 Rudder 相近的结构，而不是突然改成另一种完全不同的前端组织方式。
...
换句话说，Route 层决定“请求进来了”，Service 层决定“Rudder 语义是什么”。
...
如果需要引入代理上下文字段，这些内容应清晰与 Rudder 兼容业务契约分开，不应混入原本业务 payload 语义。

同样地，任何由外层系统注入的 actor/context 信息，也应被视为调用元数据，而不是 Rudder 原始业务 payload 的一部分。

## 9. 数据设计策略

## 9.1 业务表保持不变

现有 Rudder 的业务表必须视为固定边界。
...
这里必须明确：Octopus 的 API 面应统一遵循 Rudder 兼容接口约束，而不是拆成“兼容业务接口”和“自定义内部接口”两套命名体系。
...
`前端把它当成 Rudder server 使用时，不需要感知后端语言已经换了。`
...
- 接口名保持与 Rudder 一致
- 若 Rudder 没有某个接口名，则当前阶段不在 Octopus 中自行补出新的 HTTP 接口名
- 外层系统需要的探测、迁移、编排能力，应优先通过 Rudder 既有接口、进程级机制或外层系统自身能力解决

因此，Octopus 的 API 兼容目标不是“业务接口兼容，内部接口另起一套名字”，而是“整个 HTTP 接口命名面尽量不偏离 Rudder”。
...
- Python server 对比现有 Rudder server 的 read API 响应
- Python server 对比现有 Rudder server 的 mutation API 行为
- payload shape 是否一致
...
Octopus 应当是 Rudder 的 Python 兼容替代实现，而不是一个名字相近但行为不同的新产品。
