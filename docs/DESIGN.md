...
Chat assistant invocation 仍不应被伪装为 heartbeat run。若上游没有把 chat assistant 调用持久化为 heartbeat run，Octopus 也不能为了可见性创建虚假的 run 记录。Step 18 只按上游方式把基础执行过程作为 `ChatMessage.transcript` 返回；内部可用 `structuredPayload.__chatTranscript` 持久化，但 API response 必须剥离该 metadata，避免把内部存储结构暴露给调用方。完整日志、指标、恢复矩阵和生产级 observability 仍按后续步骤实现。
...
本地运行产生的 agent home 位于 Octopus instance home 下的 `organizations/<orgId>/workspaces/agents/<agentWorkspaceKey>/`。默认 instance home 为 `~/.octopus/instances/default`，可通过 `OCTOPUS_HOME` 与 `OCTOPUS_INSTANCE_ID` 调整。其中 agent-private skills 按上游 `AGENT_HOME/skills/<slug>/SKILL.md` 语义落盘到 `organizations/<orgId>/workspaces/agents/<agentWorkspaceKey>/skills/<slug>/SKILL.md`，并通过 runtime skills snapshot 暴露为 `sourceClass=agent_home`、`selectionKey=agent:<slug>`。
