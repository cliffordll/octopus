...
这些能力是否通过 HTTP 暴露，不应在当前项目中过早锁死。它们属于外层系统设计问题，不是 Octopus 当前必须承诺的接口面。
...
如果需要引入代理上下文字段或最小内部状态接口 payload，这些内容应清晰与 Rudder 兼容业务契约分开，不应混入原本业务 payload 语义。
...
它们是 Octopus 为适配外层系统保留的最小内部接口，因此：
