...
各能力按 `docs/FEATURE.md` 的阶段逐步落地；Agent 实施阶段应先落地一个能够真实执行的最小闭环，再由后续阶段扩展调度恢复、更多 runtime、完整 workspace 与治理能力。尚未进入当前阶段的能力可以保留规划，但不得改写成与上游不同的模型。
...
实施边界上，Agent 阶段不能只停留于 CRUD 和配置存储：它必须同时建立首个可执行的 wakeup/run/adapter 闭环，以验证 agent 运行配置具有真实服务端语义。更完整的调度恢复、adapter 类型覆盖、workspace 生命周期和成本治理仍按后续阶段扩展。
...
5. 以 agent 为入口建立首个 wakeup、run 与 runtime adapter 可执行闭环
6. 扩展运行恢复、runtime 覆盖、workspace、cost、budget 与 chat 等服务端流程
...
只关注 CRUD API 而忽略 heartbeat、runtime、workspace、run recovery，会导致 Python 版本无法替代上游 server。Agent 首次交付必须包含可运行闭环，完整执行面能力再按后续阶段扩展。
