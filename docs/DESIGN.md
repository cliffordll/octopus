...
本地运行产生的 agent home 位于当前服务启动目录下的 `.octopus/organizations/<orgId>/workspaces/agents/<agentWorkspaceKey>/`。其中 agent-private skills 按上游 `AGENT_HOME/skills/<slug>/SKILL.md` 语义落盘到 `.octopus/organizations/<orgId>/workspaces/agents/<agentWorkspaceKey>/skills/<slug>/SKILL.md`，并通过 runtime skills snapshot 暴露为 `sourceClass=agent_home`、`selectionKey=agent:<slug>`。
