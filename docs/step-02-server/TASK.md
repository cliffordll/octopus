...
| `OCTOPUS_DATABASE_URL` | instance-scoped SQLite URL | 数据库连接；默认指向 `<OCTOPUS_HOME>/instances/<OCTOPUS_INSTANCE_ID>/db/octopus.db`，外部数据库支持 `postgresql+asyncpg://...` 与 `mysql+asyncmy://...` |
