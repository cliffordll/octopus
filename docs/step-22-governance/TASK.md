...
Step 21 已实现 cost event、cost summary 和 activity query。本步骤只补齐预算治理、quota window 与 skills analytics 等后置治理能力。
...
- `tests/contract/test_step22_budget_contract.py`：覆盖 migration、validator、enum 和 schema 默认值。
...
### 22B: Budget policy、incident 与 hard-stop enforcement
...
- `tests/contract/test_step22_budget_routes.py`：覆盖 policy upsert、overview、resolve、org/agent budget patch。
- `tests/workflows/test_step22_budget_workflow.py`：覆盖 soft incident、hard incident、approval、pause、resume、cancel queued/running runs。
...
- `tests/contract/test_step22_quota_windows.py`：覆盖 org access、unknown org、provider failure、timeout、success aggregation。
...
- `tests/contract/test_step22_skills_analytics.py`：覆盖 response shape、时间窗口、org/agent scope。
- `tests/workflows/test_step22_skills_analytics_workflow.py`：覆盖 loaded/requested/used evidence 计数。
...
1. 先执行 22A，提交 budget schema/contract/migration。
2. 执行 22B，提交 budget policy、incident 和 hard-stop。
3. 执行 22C，提交 quota windows 聚合。
4. 执行 22D，提交 skills analytics 真实归集。
...
```powershell
uv run pytest tests/contract/test_step22_budget_contract.py -q
uv run pytest tests/contract/test_step22_budget_routes.py -q
uv run pytest tests/workflows/test_step22_budget_workflow.py -q
uv run pytest tests/contract/test_step22_quota_windows.py -q
```
