...
如果 shared 模式下每个子任务都有独立执行 cwd，那么 shared 就退化成 isolated。对齐上游 Rudder 后，shared 模式应明确为“直接使用项目主工作区 cwd”，不应再叠加“每个任务一个执行目录 + workspace 长锁 + workspace 队列”这套语义。
...
- 上游 Rudder 的 shared workspace 对应 `project_primary`，不会为 shared 注入 managed `workspaceStrategy`。
...
`isolated_workspace` 表示：每个 issue/subtask 有自己的执行工作区。对齐上游 Rudder 后，Octopus 的正常 isolated 项目策略应基于 Git worktree：每个 issue 一个 worktree 和固定分支。
...
上游 Rudder 的 isolated 通过 `workspaceStrategy: { type: "git_worktree" }` 落地。Octopus 对齐时应按这个约束处理：
...
上游 Rudder 当前只把 `operator_branch` 作为 policy mode 识别和持久化；已读到的代码里没有看到自动创建或切换 operator branch/worktree 的实现。
