...
- isolated + Git 项目 cwd：创建可由 Git 原生命令验证的 issue worktree。
- 同一 issue 的后续 run 复用同一 execution workspace、cwd 和未提交文件。
- repoUrl-only：复用同一 managed checkout，不同 issue 创建不同 branch、execution workspace 和 worktree。
- 非 Git cwd：preflight 拒绝，不创建普通目录或 local_fs fallback。
