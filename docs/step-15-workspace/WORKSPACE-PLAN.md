...
执行策略
  ...
  - isolated_workspace
  - operator_branch
  - 只有存在项目主工作区或 repoUrl 时才作为可选项目策略展示
...
| 策略 | UI 状态 | 原因 |
| --- | --- | --- |
| shared workspace | disabled 或 warning | 没有项目主工作区可共享 |
| isolated workspace | disabled 或 warning | 没有 Git 主工作区或 repoUrl |
| operator branch | disabled | 没有 Git 来源 |
...
4. 调整 UI 信息架构和误导文案。
5. 修正 shared runtime cwd 解析。
6. 修正 isolated Git worktree 创建、复用和校验。
7. 补测试覆盖 shared / isolated / no project / repoUrl-only / non-Git cwd。
8. 在 Shared 和 Isolated 稳定后，再讨论 operator_branch。
