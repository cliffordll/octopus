...
作用：对照 `CLAUDE.md`、`docs/DESIGN.md`、`docs/FEATURE.md` 和上游 `D:\coding\upstream-reference`，查出偏离项目定位的实现。
...
作用：补齐上游式雇佣智能体流程，使新智能体进入组织时能遵守 organization 的审批策略，而不是只通过普通创建接口绕过治理语义。

范围：

- 新增独立规划文档 `AGENT_HIRE.md`，不写入 Bug Ledger。
- 对照上游 `POST /api/orgs/{orgId}/agent-hires` 语义确认契约、状态流转和审批副作用。
- 实现 canonical hire flow：不要求审批时直接创建 agent；要求审批时创建 `pending_approval` agent 和 `hire_agent` approval。
- 审批通过后将 linked agent 激活为可运行状态；审批拒绝后保持不可运行或终止，并记录 activity。
- `budgetMonthlyCents` 只作为 hire payload 与 agent 字段保存，真实 cost/budget 统计仍归 Step 22/23。
