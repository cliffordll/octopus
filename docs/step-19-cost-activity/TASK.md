...
本步骤只做 cost event 归集、summary/query 和 activity query；budget hard-stop、incident、quota、skills analytics 归 Step 20。
...
- 不做 budget policy、budget incident、hard-stop、approval 副作用，归 Step 20。
- 不做 provider quota 真实读取，归 Step 20。
- 不做 skills analytics 真实归集，归 Step 20。
