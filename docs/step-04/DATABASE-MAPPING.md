...
- 修改 `server/config.py` `server/lifespan.py` `server/services/orgs.py` `server/routes/orgs.py`,接入数据库引擎与会话依赖
...
`issues_open_automation_execution_uq` 是带 WHERE 子句的部分唯一索引,SQLAlchemy 表达方式较复杂,且不影响 Step 4 的基础映射验证,本次暂未实现,留到 Step 6/7 真正用到 automation execution 流转时再补。
...
route 通过 FastAPI `Depends` 注入 session：
...
- 新增 `get_session(request)` async generator dependency,从 `app.state.session_factory` 创建 session 并 yield
- 改 `get_org_service` 接收 session 入参,返回 `OrgService(session)`
...
`tests/contract/test_step4_database.py` 共 9 个测试：
...
加上 Step 2 / Step 3 已有的 30 个测试,`tests/contract/` 现共 39 个测试全部通过。
