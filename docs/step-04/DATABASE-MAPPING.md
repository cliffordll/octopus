...
**Step 4 第一版遗漏**:lifespan 只建 engine / session_factory,没建表。fixture-based 测试不暴露(fixture 显式调 `Base.metadata.create_all`),真启动 server 后第一个碰 DB 的请求即 `sqlite3.OperationalError: no such table` → HTTP 500。Step 6 curl 实测暴露后由 `fix/lifespan-auto-create-schema` 补:lifespan 在 `database_url` 命中 `sqlite` 时自动 `Base.metadata.create_all`(`create_all` 幂等,只建缺的表)。PostgreSQL 部署不触发,等真正的迁移工具(alembic)落地后通过迁移建表。回归测试见 `tests/contract/test_lifespan_schema_bootstrap.py`。
