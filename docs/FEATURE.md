...
| 22 | Runtime Tool & Output Bug Fix | `docs/step-22-runtime-fix/` |
...
- 目标：集中修复本地 runtime 执行闭环里的两个阻塞 bug：tool schema 注入不清导致 OpenCode 工具调用失败，以及 stdout/stderr/json event 不能实时落库导致 UI 长时间看不到执行过程。
- 交付：runtime tool capability 描述、按 runtime 注入的工具使用约束、OpenCode `bash` 等内置工具 schema guidance、工具调用错误归一化、stdout/stderr/json event 实时 ingestion、错误优先级和回归测试。
- 验收：本地 runtime 能获得明确工具能力与参数约束；工具 schema 错误可诊断；stdout/stderr 与 assistant/tool events 在进程运行期间持续写入 run events/log，不等待进程结束或长时间 buffer flush。
...
- Runtime tool capability/schema 与实时 stdout/stderr event ingestion 首轮 bug fix 归 Step 22：本地 runtime adapter 如何把 OpenCode/Codex/Claude 的工具能力、参数约束和错误解释注入到运行上下文，并持续采集运行期输出。
