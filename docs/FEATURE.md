...
这一阶段的目标不是尽快堆出业务代码，而是先把 Octopus 作为 Rudder 服务端 Python 兼容式重写项目的基础边界固定下来，确保后续实现不会偏离设计文档。
...
- B 如果发现契约缺口或契约与 Rudder 不一致，先反馈 A 更新契约文档，再继续实现
...
- 契约冲突由 A 基于 Rudder 对照结果裁定
- 实现冲突由 B 处理，但不得突破已合入主干的契约边界
- 涉及兼容语义的冲突，必须先停下来核对 Rudder，不能直接拍脑袋合并

## 3. 工作原理与设计意图

Octopus 的本质是对 Rudder 服务端的兼容式重写，因此开发计划必须围绕以下原则推进：

- 外部契约以 Rudder 为准，不以 Python 习惯为准
- 先固定契约和结构，再写实现
- 先做只读和基础设施，再做复杂工作流
...
- organization 分配系统
- 前端重写
- 与 Rudder 兼容目标无关的 CLI、桌面或外围工具实现
...
- 明确哪些表属于 Rudder 业务表，哪些新增表只允许是基础设施表

验收 demo：

- Demo 1：展示 `packages/database/` 目录结构，确认 schema、clients、queries、migrations 职责分离
- Demo 2：选取 1 到 2 个 Rudder 业务表，展示 Python 映射定义，并说明字段如何对应 Rudder
...
- 选择带 organization 语义的 Rudder 已有接口进行验证，例如：
...
- 与 Rudder 对照的 read contract 测试

验收：

- 能对照 Rudder 响应结构检查字段、nullability、默认行为

验收 demo：

- Demo 1：选取 1 到 2 个只读接口做实际调用，展示请求方式和响应结果
- Demo 2：逐字段对照 Rudder 返回结构，展示字段名、nullability、默认值和排序/过滤行为是否一致
...
- conversation、message、attachment、context link、convert-to-issue 等核心能力与 Rudder 对齐

验收 demo：

- Demo 1：演示一次最小 chat 会话创建、消息写入和消息流转
- Demo 2：演示一次关键动作，例如 chat 转 issue、context link 处理或 attachment 关联
- Demo 3：对照 Rudder 展示消息顺序、字段结构和关键状态语义的一致性
...
- 冲突处理是否经过 Rudder 对照，而不是直接本地拍平

## 10. 当前约束与风险

当前最主要的约束和风险包括：

- Rudder 的真实行为可能比设计文档更复杂，后续必须持续对照源码
...
Octopus 的正确推进方式不是“先做一版能跑”，而是“先做一版不会把项目带偏的基础”，再逐步替换 Rudder 服务端能力。
