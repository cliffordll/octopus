...
`base -> server -> contract -> db -> scope -> orgs -> issues -> review -> approvals -> projects -> agent-execution-and-conversation -> goals -> run-hardening -> runtime-expansion -> workspace -> chat-expansion -> org-resources-skills -> chat-refs -> storage -> debug-visibility -> closed-loop-qa -> runtime-bug-fix -> database-portability -> agent-memory -> activity-query -> hardening -> cost-summary -> governance -> plugins -> access`
...
- Agent-private skill 管理 UI 归 Step 17 follow-up：单个 agent 的私有 skill 创建、编辑、启用、停用和描述展示，应复用 Step 14/17 的服务端契约，不作为 Step 24 memory 或 Step 29 plugin framework 的一部分。
