...
- 补齐 `claude_local` managed HOME/credential 注入；run-time skills ephemeral 注入已完成。
- 补齐 `opencode_local` managed HOME/credential 和 model requirement 深化；run-time skills 注入已完成。
