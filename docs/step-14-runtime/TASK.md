...
- Agent-private skill 已按上游 `AGENT_HOME/skills/<slug>/SKILL.md` 语义文件化；Python 版当前 `AGENT_HOME` 解析到服务启动目录下 `.octopus/organizations/<orgId>/workspaces/agents/<agentWorkspaceKey>/`，因此实际文件路径为 `.octopus/organizations/<orgId>/workspaces/agents/<agentWorkspaceKey>/skills/<slug>/SKILL.md`。返回 entry 使用 `selectionKey=agent:<slug>`、`sourceClass=agent_home`、`origin=user_installed`、`state=external`；enable 后可被 Codex materialize 到 managed `CODEX_HOME/skills/<slug>`。
