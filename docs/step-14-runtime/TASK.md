...
- 继续补齐 `codex_local` 上游运行细节：credential home sync、unknown session resume retry、local agent JWT/env context 注入，以及更完整的 provider/biller 识别。
...
- 继续深化 skills snapshot/sync：Codex materialization、private skill 文件化、默认 managed `CODEX_HOME` 注入和 loaded skills 元数据已具备基础闭环；后续还需补齐 credential home sync 以及 Claude/OpenCode run 时 ephemeral mount 的真实注入。
