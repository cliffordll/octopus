...
- 继续深化 skills snapshot/sync：Codex materialization 已具备基础闭环，后续还需补齐 credential home sync、loaded skills 元数据、private skill 文件化以及 Claude/OpenCode run 时 ephemeral mount 的真实注入。
- 将 private skill 从兼容响应升级为上游证据完整的持久化 skill 文件/entry 管理。
...
- `codex_local` 已支持 desired bundled/organization skill materialize 到 managed `CODEX_HOME/skills`；后续仍需在执行结果中补齐 loaded skills 元数据，保证 run 记录能说明实际加载了哪些 skill。
...
- `POST /api/agents/{id}/skills/private` 当前只返回兼容 entry；后续需要落地为 agent home skill 文件，能被 snapshot 发现、enable，并参与 Codex materialization 或 Claude/OpenCode ephemeral mount。
