...
- Live hello probe、HTTP endpoint live probe、OpenCode model live availability、Claude/OpenCode session resume 与完整 runtime compatibility hardening 归 Step 20。
- Local agent JWT/API key、secret/env binding、真实 actor/access 和 instructions 编辑权限深化归 Step 19。
- 真实 quota provider 读取、provider/biller 成本治理联动和 skills analytics 真实归集归 Step 16。
