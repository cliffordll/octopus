...
Step 16 仍负责 cost、budget、activity 查询与治理联动；Step 14 只提供 adapter quota window 的可调用兼容入口和运行结果中的 usage 基础归一化，不在本步骤实现真实 provider quota 聚合或治理。
...
- `codex_local` 检查 cwd、command resolvable 和认证线索；CLI hello probe 归 Step 20。
- `claude_local` 检查 cwd、command resolvable 和 API key/subscription auth 线索；CLI hello probe 归 Step 20。
- `opencode_local` 检查 cwd、command resolvable、model discovery 和基础 model 配置；model live availability 与 hello probe 归 Step 20。
- `http` 检查 url、method 和协议形态；endpoint live probe 归 Step 20。
...
- `claude_local`、`codex_local`、`opencode_local` 需要注入上游兼容 runtime env 与 agent/run context；local agent JWT/API key、真实 secret/env 解析归 Step 19。
...
- 新增 `GET /api/agents/{id}/skills/analytics`，返回 agent skills 使用 analytics 兼容结构；Step 14 可返回空统计，真实 loaded/requested/used 归集归 Step 16。
...
- 本步骤只负责 adapter 侧可调用结构、失败标准化输出与 contract 稳定性；真实 provider quota 读取、预算限制、治理联动、查询聚合仍归 Step 16。
...
- registry 需要暴露 adapter metadata：models/listModels、skills capability、environment test、quota window capability、agent configuration doc，以及 Step 19 可接入的 auth/secret 能力边界。
...
- `opencode_local` 已按上游 agent configuration 要求强制 `agentRuntimeConfig.model` 使用 `provider/model` 格式；agent 创建/更新路径返回 422，environment test 返回基础 model 配置检查结果。真实 model live availability 仍归 Step 20。
...
- 新增 `/api/agents/{id}/skills/analytics`，返回兼容结构；Step 14 没有真实归集数据时返回空统计，不自造使用量，真实统计归 Step 16。
- 扩展 runtime adapter contract，增加 metadata/config doc 暴露，包含 models、skills、environment test、quota window、agent configuration doc 和 Step 19 可接入的 auth/secret 能力边界声明。
...
- `codex_local`、`claude_local`、`opencode_local` 注入上游兼容 runtime env 与 agent/run context；local agent JWT/API key、真实 secret/env 解析仍留给 Step 19。
- 将 environment test 升级为基础真实 probe：cwd、command resolvable、auth 线索和 HTTP URL 形态；live hello probe、HTTP endpoint live probe 归 Step 20。
...
- `claude_local` runtime context env、managed HOME/credential 与 run-time skills 注入已完成。session resume 归 Step 20，local agent JWT/API key 归 Step 19。
- `opencode_local` model requirement、runtime context env、managed HOME/credential 与 run-time skills 注入已完成。session resume 归 Step 20，local agent JWT/API key 归 Step 19。
- 继续补齐 `codex_local` 上游运行细节中属于 adapter baseline 的部分；真实 auth/secret/API key 绑定归 Step 19，更多 provider/biller 治理归 Step 16。
- 继续深化 environment test：cwd、command、auth 线索和 HTTP URL 基础检查已完成；live hello probe 与 HTTP endpoint live probe 归 Step 20。
- 将 model discovery 继续深化到 environment test 的 OpenCode model configured/available probe；Codex 当前保持上游静态模型清单，OpenCode `opencode models` 发现与缓存已完成。
- 继续深化 skills snapshot/sync：Codex materialization、private skill 文件化、默认 managed `CODEX_HOME`/`HOME` 注入、credential home sync、runtime env/context 注入、unknown session retry、loaded skills 元数据，以及 Claude/OpenCode run 时 skills 注入已具备基础闭环。
- `managed instructions bundle materialization` 与 instructions path 解析边界已完成；后续只保留更深的编辑/权限治理扩展，真实 actor/access 归 Step 19。
- adapter quota window 在 Step 14 保持可解释失败结构和可调用 contract；真实 provider quota 读取与聚合归 Step 16。
...
- Quota window 的真实 provider 读取、归集、展示和治理联动归 Step 16；adapter 侧 quota window 兼容入口和失败结构属于本步骤。
- Local agent JWT/API key、secret/env binding 和真实 actor/access 归 Step 19。
- Claude/OpenCode session resume、HTTP/CLI live hello probe、HTTP endpoint live probe 和完整 runtime compatibility hardening 归 Step 20。
