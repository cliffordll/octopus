...
- 继续深化 `claude_local` session resume、runtime context env、local agent JWT/API key 兼容入口；managed HOME/credential 与 run-time skills 注入已完成。
- 继续深化 `opencode_local` model requirement、session resume、runtime context env、local agent JWT/API key 兼容入口；managed HOME/credential 与 run-time skills 注入已完成。
- 继续补齐 `codex_local` 上游运行细节：local agent JWT/API key 兼容入口，以及更多非 OpenRouter provider 细节（如未来上游扩展）。
- 继续深化 environment test：cwd、command、auth 线索和 HTTP URL 基础检查已完成；后续补 hello probe、OpenCode model probe、HTTP endpoint live probe。
