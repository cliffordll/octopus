...
- 将 `claude_local` 从 process 薄封装重构为上游兼容 adapter，补齐 stream-json、session、usage、login/error、HOME/credential、skills 注入。
- 将 `opencode_local` 从 process 薄封装重构为上游兼容 adapter，补齐 `opencode run --format json`、model requirement、session、usage、auth/model error、HOME/credential、skills 注入。
- 补齐 `codex_local` 上游运行细节：managed CODEX_HOME、credential sync、benign stderr、unknown session/tool error、loaded skills、biller/usage。
...
- 将 skills snapshot/sync 从空 entries 升级为 runtime skills entries 扫描、desired selection、installed/external/missing 状态和 persistent/ephemeral mode。
- 新增 `/api/agents/{id}/skills/enable`、`/api/agents/{id}/skills/private`、`/api/agents/{id}/skills/analytics`。
- 补齐新建本地 runtime agent 的 managed instructions bundle materialization 和 instructions path 更新边界。
- 补齐 adapter quota window probe 能力，供 Step 16 治理使用。
- 补齐 adapter configuration doc / metadata 暴露，避免 UI/CLI 只能硬编码 runtime 配置说明。
