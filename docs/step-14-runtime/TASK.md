...
- 继续深化 `claude_local` runtime context env；managed HOME/credential 与 run-time skills 注入已完成。session resume 归 Step 20，local agent JWT/API key 归 Step 19。
- 继续深化 `opencode_local` model requirement 与 runtime context env；managed HOME/credential 与 run-time skills 注入已完成。session resume 归 Step 20，local agent JWT/API key 归 Step 19。
