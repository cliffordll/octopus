...
- `tests/contract/test_step20_budget_contract.py`：覆盖 migration、validator、enum 和 schema 默认值。
...
### 20B: Budget policy、incident 与 hard-stop enforcement
...
- `tests/contract/test_step20_budget_routes.py`：覆盖 policy upsert、overview、resolve、org/agent budget patch。
- `tests/workflows/test_step20_budget_workflow.py`：覆盖 soft incident、hard incident、approval、pause、resume、cancel queued/running runs。
...
- `tests/contract/test_step20_quota_windows.py`：覆盖 org access、unknown org、provider failure、timeout、success aggregation。
...
- `tests/contract/test_step20_skills_analytics.py`：覆盖 response shape、时间窗口、org/agent scope。
- `tests/workflows/test_step20_skills_analytics_workflow.py`：覆盖 loaded/requested/used evidence 计数。
...
1. 先执行 20A，提交 budget schema/contract/migration。
2. 执行 20B，提交 budget policy、incident 和 hard-stop。
3. 执行 20C，提交 quota windows 聚合。
4. 执行 20D，提交 skills analytics 真实归集。
...
```powershell
uv run pytest tests/contract/test_step20_budget_contract.py -q
uv run pytest tests/contract/test_step20_budget_routes.py -q
uv run pytest tests/workflows/test_step20_budget_workflow.py -q
uv run pytest tests/contract/test_step20_quota_windows.py -q
```
